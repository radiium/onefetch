...
export const createDownloadListState = (props: {
	status?: DownloadStatus[];
	type?: DownloadType[];
}) => {
	let status = $state<DownloadStatus[]>(props.status ?? []);
	let type = $state<DownloadType[]>(props.type ?? []);
	...
	return {
		...
		// State
		get current() {
			return getAllState.current;
		},
		get loading() {
			return getAllState.loading;
		},
		get error() {
			return getAllState.error;
		},
		get,
		resetFilters() {
			status = [];
